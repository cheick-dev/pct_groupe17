...
function TarifForm({
  ...
  const form = useForm<TarifFormData>({
    ...
    defaultValues: {
      ...
      FraisDossier:
        tarif && tarif.FraisDossier ? tarif.FraisDossier.toString() : "0",
  ...
  const onSubmit = async (data: TarifFormData) => {
    ...
    try {
      const res =
        tarif && tarif.ID_FraisTimbre
          ? await updateTarif(formData, tarif.ID_FraisTimbre)
