...
import {
  ...
  getTarifByType,
  getTarifs,
} from "@/server/admin/tarif/tarif";
import { Eye } from "lucide-react";
import { FraisTimbre } from "@/lib/generated/prisma";

function page() {
  const [tarifs, setTarifs] = useState<any>([]);
  const [tarifSelected, setTarifSelected] = useState<FraisTimbre | undefined>(
    undefined
  );
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {tarifs.map((tarif: any) => (
                <tr
                  key={tarif.ID_FraisTimbre}
                  className="border-b hover:bg-gray-50"
                >
                  <td className="py-3 px-4">{tarif.TypeActe}</td>

                  <td className="py-3 px-4">{tarif.PrixTimbre}</td>
                  ...
                  <td className="py-3 px-4 text-right">
                    <Button
                      onClick={async () => {
                        ...
                        setTarifSelected({
                          ...res,
                          ID_FraisTimbre: tarif.ID_FraisTimbre,
                        });
                        setIsOpen(true);
                      }}
                    >
